...
| `cardano_census_pool_stake_ratio{pool_id,ticker,name,reach,pools,relays_total,relays_reachable,relays_reversed,relays}` | Stake of the operators with the most stake among pools not fully reachable, at most `--top-pools` rows. `relays_reversed` counts relays the pool index matched only through their octet reversal |
