...
The module runs the census every 15 minutes as a oneshot service under a dynamic user and writes to `/var/lib/cardano-census`. `interval`, `timeout`, `parallel`, `forkTolerance`, `labels`, `poolIndex`, `topPools`, `reportFile`, `reportRetentionDays`, `textfileDirectory` and `extraArgs` are options. After each run the report is copied zstd-compressed into `reports/` beside it, named by the run's UTC time, and copies older than `reportRetentionDays` are deleted, 90 by default; 0 keeps only the latest.
